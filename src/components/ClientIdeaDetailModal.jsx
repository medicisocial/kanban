...
export default function ClientIdeaDetailModal({
  ...
  const [referenceVideo, setReferenceVideo] = useState(idea?.referenceVideo || '');
  const [notes, setNotes] = useState(idea?.description || '');
  const [error, setError] = useState('');

  useEffect(() => {
    setTitle(idea?.title || '');
    setContentType(idea?.contentType || 'Reel');
    setReferenceVideo(idea?.referenceVideo || '');
  ...
  const slideType = isSlidePostType(contentType || idea.contentType);
  const normalizedRef = normalizeLink(referenceVideo.trim()) || '';
  const dirty =
    title.trim() !== String(idea.title || '').trim() ||
    contentType !== (idea.contentType || 'Reel') ||
    normalizedRef !== String(idea.referenceVideo || '').trim() ||
    notes.trim() !== String(idea.description || '').trim();

  const buildDetailsUpdates = ({ includeNotes = true } = {}) => ({
    title: title.trim() || 'New idea',
    contentType,
    referenceVideo: normalizedRef,
    ...(includeNotes ? { description: notes.trim() } : {}),
  });

  const handleSave = async () => {
    setError('');
    try {
      await onSave?.(idea.id, buildDetailsUpdates({ includeNotes: true }));
      onClose?.();
    } catch (err) {
      setError(err?.message || 'Could not save this idea.');
    }
  };
  ...
  const handleApprove = async () => {
    setError('');
    try {
      if (dirty) await onSave?.(idea.id, buildDetailsUpdates({ includeNotes: true }));
  ...
  const handleReject = async () => {
    ...
    setError('');
    try {
      // Persist details without writing rejection text into description.
      if (
        title.trim() !== String(idea.title || '').trim() ||
        contentType !== (idea.contentType || 'Reel') ||
        normalizedRef !== String(idea.referenceVideo || '').trim()
      ) {
        await onSave?.(idea.id, buildDetailsUpdates({ includeNotes: false }));
      }
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          {activeTab === 'details' && (
            <div className="space-y-4">
              <label className="block">
                <span className="mb-1.5 block text-xs font-medium text-white/45">Title</span>
                <input
                  type="text"
                  value={title}
              ...
              <label className="block">
                <span className="mb-1.5 block text-xs font-medium text-white/45">Type</span>
                <select
                  value={contentType}
                  onChange={(event) => setContentType(event.target.value)}
                  className={`${selectClass} w-full`}
                >
                  {CONTENT_TYPES.filter((type) => type !== 'Story').map((type) => (
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(event) => setReferenceVideo(event.target.value)}
          ...
          {activeTab === 'script' && (
            <div>
              {slideType ? (
                <PostSlidesPanel
                  contentType={contentType || idea.contentType}
              ...
              <p className="mt-3 text-[10px] text-white/35">
                Script is view-only. Edit details and notes to send updates to your team.
              </p>
            </div>
          )}
