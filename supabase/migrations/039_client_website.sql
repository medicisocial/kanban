-- Optional public website URL per client brand profile.
...
begin
  ...
  insert into public.client_records (
    ...
    photo_gallery_link, website, business_type, account_manager, videographer, photographer,
  ...
  on conflict (org_id, brand_key) do update
    set
      ...
      website = case when p_patch ? 'website' then excluded.website else client_records.website end,
...
as $$
  select jsonb_build_object(
    ...
    'website', nullif(trim(cr.website), ''),
