-- Per-client videographer/photographer assignees + separate carousel/static counts.
-- Keep carousel_static_target synced as carousel + 0.5 * static for deliverables.

alter table public.client_records
  add column if not exists videographer text not null default '';

alter table public.client_records
  add column if not exists photographer text not null default '';

alter table public.client_records
  add column if not exists carousel_target integer not null default 0;

alter table public.client_records
  add column if not exists static_target integer not null default 0;

-- Backfill: treat existing feed budget as whole carousels (statics unknown).
update public.client_records
set
  carousel_target = greatest(0, round(coalesce(carousel_static_target, 0))::integer),
  static_target = 0
where coalesce(carousel_target, 0) = 0
  and coalesce(static_target, 0) = 0
  and coalesce(carousel_static_target, 0) > 0;
