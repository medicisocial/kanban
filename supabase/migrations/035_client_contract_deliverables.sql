-- Contract deliverables: reel points quota + carousel/static count per client.
-- Legacy deliverable_target remains but app code stops using it.

alter table public.client_records
  add column if not exists reel_points_target numeric(6,1) not null default 0;

alter table public.client_records
  add column if not exists carousel_static_target integer not null default 0;
...
begin
  ...
  values (
    ...
    coalesce((p_patch->>'carouselStaticTarget')::integer, 0),
