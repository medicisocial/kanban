-- Persist brand company-file delete tombstones in client_records (was localStorage-only).

alter table public.client_records
  add column if not exists deleted_company_file_ids jsonb not null default '[]'::jsonb;
...
as $$
  select jsonb_build_object(
    ...
    'deletedCompanyFileIds', coalesce(cr.deleted_company_file_ids, '[]'::jsonb)
  )
...
-- Legacy blob vault RPC superseded by patch_portal_password_vault on portal_password_vault.
drop function if exists public.patch_clients_portal_password_vault(text, text, jsonb);
