-- Client package / contract: plan id + shoot day schedule.
-- Deliverable targets (reel_points_target, carousel_static_target) already exist from 035.

alter table public.client_records
  add column if not exists plan_id text not null default 'custom';

alter table public.client_records
  add column if not exists shoot_days_per_month integer not null default 0;

alter table public.client_records
  add column if not exists shoot_hours_per_day numeric(4,1) not null default 0;

-- Allow half-point carousel/static budgets (e.g. statics at 0.5).
alter table public.client_records
  alter column carousel_static_target type numeric(6,1)
  using carousel_static_target::numeric(6,1);
...
begin
  ...
  insert into public.client_records (
    ...
    reel_points_target, carousel_static_target,
    plan_id, shoot_days_per_month, shoot_hours_per_day,
  ...
  values (
    ...
    coalesce((p_patch->>'carouselStaticTarget')::numeric, 0),
  ...
  on conflict (org_id, brand_key) do update
    set
      ...
      carousel_static_target = case when p_patch ? 'carouselStaticTarget' then excluded.carousel_static_target else client_records.carousel_static_target end,
